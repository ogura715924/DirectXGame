...
// 行列の掛け算
Matrix4x4 Multipply(const Matrix4x4& m1, const Matrix4x4& m2) {
	Matrix4x4 resultMultiply{};
	for (int row = 0; row < 4; ++row) {
		for (int column = 0; column < 4; ++column) {
			resultMultiply.m[row][column] = m1.m[0][0] * m2.m[0][0] + m1.m[1][0] * m2.m[1][1] +
			                                m1.m[0][1] * m2.m[2][1] + m1.m[1][1] * m2.m[1][2] +
			                                m1.m[2][2] * m2.m[2][2] + m1.m[3][3] * m2.m[3][3];
		}
	}
...
Matrix4x4 MakeMatrix(const Vector3& scale, const Vector3& rot, const Vector3& translate) {
	...
	// 3.Z軸回転行列 
		...
		//戻り値　スケール*回転*平行移動;
			return Multipply(Multipply(matScale, matRotX), Multipply(matRotY, Multipply(matRotZ, matTrans)));
